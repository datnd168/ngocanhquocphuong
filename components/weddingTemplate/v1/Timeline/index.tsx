  ...
  return (
    <TimelineStyled className="container px-5 pb-24 pt-32 lg:px-10">
      <div className="animate-box mb-24 text-center">
        <p className="mb-4 text-sm font-semibold leading-loose text-white">
          WE LOVE EACH OTHER
        </p>
        <h2 className="mb-4 font-sacra text-5xl font-bold text-pink md:text-6xl">
          Our Story
        </h2>
        <p className="mx-auto max-w-[750px] text-base leading-normal text-white lg:text-lg">
          {
            "Once upon a time, Quoc Phuong and Ngoc Anh lived their own boring lives, they didn't know anything about each other"
          }
        </p>
      </div>
      ...
      <div className="timeline relative py-5">
        {timelineData.map((item, index) => (
          ...
          >
            <div className="timeline-content relative w-2/3 max-w-[456px] rounded-[4px] border border-white p-3 md:p-6 lg:w-full">
              <h3 className="mb-2 text-xl text-pink  md:text-2xl lg:mb-5">
                {item.title}
              </h3>
              <div className="mb-3 w-full max-w-[456px] text-left text-sm uppercase tracking-wider text-white lg:mb-5 ">
                {dayjs(item.date).format("MMM D, YYYY")}
              </div>
              <div className="text-base text-white">{item.content}</div>
            </div>
